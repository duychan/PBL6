...
async def pred(content: Content):
    ...
    return {"result": (output[0]) if output is not None else None, "confidence": np.round(conf, 4)*100}
...
async def pred(content: Content):
    ...
    return {"result": (output[0]) if output is not None else None}


@app.post("/predict_nb/")
async def pred(content: Content):
    model = pickle.load(open("../Model_NB/Word2Vec_pickle_NB.pkl", "rb"))
    output = model.predict(content.content)
    return {"result": (output[0]) if output is not None else None}
